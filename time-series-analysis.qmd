...
instrumentalness_line_plot <- ggplot(data = spotify_mean_yearly, aes(x = year, instrumentalness_year_mean))+
  ...
  labs(
    ...
    title = "Yearly time series of Instrumentalness"
  )
...
pop_line_plot <- ggplot(data = spotify_mean_monthly, aes(x = month, y = popularity_month_mean)) +
  geom_line() + 
  labs(
    x = "Year",
    y = "Song Popularity Mean",
...
loud_line_plot <- ggplot(data = spotify_mean_monthly, aes(x = month, y = loudness_month_mean)) +
  geom_line() + 
  labs(
    x = "Year",
    y = "Song Loudness Mean",
...
acoustic_line_plot <- ggplot(data = spotify_mean_monthly, aes(x = month, y = acousticness_month_mean)) +
  geom_line() + 
  labs(
    x = "Year",
    y = "Song Acousticness Mean",
...
energy_line_plot <- ggplot(data = spotify_mean_monthly, aes(x = month, y = energy_month_mean)) +
  geom_line() + 
  labs(
    x = "Year",
    y = "Song Energy Mean",
...
danceability_line_plot <- ggplot(data = spotify_mean_monthly, aes(x = month, danceability_month_mean))+
  geom_line() + 
  labs(
    x = "Year",
    y = "Song Danceability Mean",
...
liveness_line_plot <- ggplot(data = spotify_mean_monthly, aes(x = month, liveness_month_mean))+
  geom_line() + 
  labs(
    x = "Year",
    y = "Song Liveness Mean",
...
duration_line_plot <- ggplot(data = spotify_mean_monthly, aes(x = month, duration_ms_month_mean))+
  geom_line() + 
  labs(
    x = "Year",
    y = "Song Duration Mean",
...
instrumentalness_line_plot <- ggplot(data = spotify_mean_monthly, aes(x = month, instrumentalness_month_mean))+
  geom_line() + 
  labs(
    x = "Year",
    y = "Song Instrumentalness Mean",
    title = "Monthly time series of Instrumentalness"
  )
...
```{r}
# examples of EDA at each granularity (to get rid of)
# or we could put very small versions of these plots in the EDA to justify monthly granularity
ggplot(data = spotify_mean_daily, aes(x=popularity_daily_mean, y=energy_daily_mean)) +
  geom_point() + 
  geom_smooth(color="blue", alpha=0.75, se=FALSE, method="lm") +
  geom_smooth(color="red", alpha=0.75, se=FALSE)

ggplot(data = spotify_mean_monthly, aes(x=popularity_month_mean, y=energy_month_mean)) +
  geom_point() + 
  geom_smooth(color="blue", alpha=0.75, se=FALSE, method="lm") +
  geom_smooth(color="red", alpha=0.75, se=FALSE)

ggplot(data = spotify_mean_yearly, aes(x=popularity_year_mean, y=energy_year_mean)) +
  geom_point() + 
  geom_smooth(color="blue", alpha=0.75, se=FALSE, method="lm") +
  geom_smooth(color="red", alpha=0.75, se=FALSE)
```
...
```{r}
forecast::ggCcf(spotify_mean_monthly$energy_month_mean, spotify_mean_monthly$popularity_month_mean)



#monthly cross correlation check for energy on popularity 

energy_lags = spotify_mean_monthly %>%
...
#yearly cross correlation check for energy on popularity 
energy_lags = spotify_mean_yearly %>%
  select(year, energy_year_mean, popularity_year_mean) %>%
  mutate(
    "lag 0" = lag(energy_year_mean,0),
    "lag 1" = lag(energy_year_mean,1),
    "lag 2" = lag(energy_year_mean,2),
    "lag 3" = lag(energy_year_mean,3),
    "lag 4" = lag(energy_year_mean,4),
    "lag 5" = lag(energy_year_mean,5),
    "lag 6" = lag(energy_year_mean,6),
    "lag 7" = lag(energy_year_mean,7),
    "lag 8" = lag(energy_year_mean,8),
    "lag 9" = lag(energy_year_mean,9),
    "lag 10" = lag(energy_year_mean,10),
    "lag 11" = lag(energy_year_mean,11)
  ) 
corrs = energy_lags %>%
  select(-energy_year_mean, -year) %>%
  tidyr::gather(lag, energy_year_mean, -popularity_year_mean) %>%
  group_by(lag) %>%
  summarize(corr = cor(energy_year_mean, popularity_year_mean, use="complete.obs"))
energy_lags %>%
  select(-energy_year_mean, -year) %>%
  tidyr::gather(lag, energy_year_mean, -popularity_year_mean) %>%
  ggplot(aes(x=energy_year_mean, y=popularity_year_mean)) +
    geom_point(alpha=0.3) +
    facet_wrap(~forcats::as_factor(lag)) +
    #geom_line(stat="smooth", method="loess", color='red',  size=1.2, alpha=0.75) +
    geom_line(stat="smooth", method="lm", color="blue", size=1.2, alpha=0.75) +
...
#printing just correlations of energy on popularity 
energy_lags = spotify_mean_monthly %>%
  select(month, energy_month_mean, popularity_month_mean) %>%
  mutate(
    "lag 0" = lag(energy_month_mean,0),
...
#checking lag correlations without the 0's in popularity 
...
#it did not help, likely suggestss we don't lag this predictor)
```
...
```{r}
model1 <- lm(popularity_month_mean ~ danceability_month_mean + duration_ms_month_mean + energy_month_mean + explicit_month_mean +  instrumentalness_month_mean + key_month_mean + liveness_month_mean + loudness_month_mean + mode_month_mean +  speechiness_month_mean + tempo_month_mean + valence_month_mean, data = spotify_mean_monthly) 
