...
Key interactions from RFE and their model coefficients are here:
...
The Neural Network performs better than the Linear Regression, with its R squared of 0.82 and RMSE of 84.2. The RMSE appears to be high in both the linear regression and neural network due to some especially high residuals that come from the recent 2020s data. This network, while not fully tuned, is meant to represent an upper bound benchmark for how well we can model popularity with song metrics.

There is a clear performance boost from adding temporal information, which makes sense as many variables even changed sign in terms of their coefficients from earlier decades to later decades in the model, suggesting a shift in its impact on popularity. Additionally, music itself gained in popularity score, which also changes its relationships with other variables. In the simplified regression approach, it seemed as though different relationships between the target and a metric over different decades led to an "averaging" effect over coefficients not allowing any of the relationships to be learned. Thus, this could be benefited by including interaction effects by decade for more nuanced relationships between metrics and popularity scores.

The Neural Network's performing better than the interaction regression shows there is still more left to be desired in the interpretable regression. There may be complex non-linear relationships the network finds, for example, transforms or potentially even more structured change points in the relationships between song metrics and popularity. Further feature engineering and selection should ideally be performed to increase the interpretable model's performance relative to the neural network to eke out as much information as possible on the relationships between metrics and song popularity. More specifically, future work on this project would definitely include change point detection and analysis to further boost the interpretable modeling.
...
In the model fitting part of our analysis, we had two goals:
1) Track change in popularity over the months using other variable means through ARIMA (done in earlier section)
2) Use temporal relationships between metrics (found from prior analysis or feature selection with RFE) to predict popularity at the individual scale rather than the monthly scale. This accounts for nuance of the music in decades. Ultimately, the modeling done demonstrates the validity of many of the metric relationships found earlier in the report as a predictor of not merely monthly song popularity, but individual song popularity. 
