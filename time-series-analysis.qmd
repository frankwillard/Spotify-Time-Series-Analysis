...
For the final step of our EDA, we checked the cross correlation function and correlation coefficients at lags 0-11 for all predictors. Only tempo resulted in an improvement in correlation coefficient, which is shown below. ALl others either did not show any lagged correlation, or correlation was lower with increased lags. Lagging at month 10 for tempo showed a marked decrease in RMSE, AIC and BIC. 
...
forecast::ggCcf(spotify_mean_monthly$tempo_month_mean, 
                ...
                main = "CCF plot for tempo lagged on popularity")
...
The third model performed even better than the prior two, with an root mean squared error or `r round(model3_rmse, 3)` and an AIC of `r round(model3_aic, 3)`. The temporal components improved the model, but we can include an ARIMA component on the errors instead of the cross correlation terms we added in the second and third models.
...
The ARIMA model included alongside the multiple linear regression performed the best in terms of AIC, yielding an AIC of `r round(model4_aic, 3)`.
